							...
							<div class="x_panel">
								...
								<div class="x_content">
									...
									<form class="form-horizontal form-label-left" id="urlform">
										...
										<div class="form-group">
											<label class="control-label col-md-3 col-sm-3 col-xs-3">내용
											</label>
											<div class="col-md-9 col-sm-9 col-xs-9">
										...
										<div class="form-group">
											...
											<div class="col-md-9 col-sm-9 col-xs-9">
												<select id="selectUrlflag" class="form-control">
													<option>선택</option>
													<option value="-1">예외처리(부분일치)</option>
													<option value="-2">블랙리스트(부분일치)</option>
													<option value="1">예외처리(전체일치)</option>
													<option value="2">블랙리스트(전체일치)</option>
												</select>
											</div>
										</div>
										...
										<div class="form-group">
											<div class="col-md-9 col-md-offset-3">
												<button type="submit" id="urlformsubmit"
	...
	<script>
																													$(document).ready(function() {
																														...
																														$("#urlform").on("submit", function(e) {
																															...
																															var flag = $("#selectUrlflag").val();
																															var content = $("#inputContent").val();

																															$.ajax({
																																type : "post",
																																url : "/nmds-web/rest/urlrule",
																																data : {
																																	url : url,
																																	content : content,
																																	flag : flag
																																...
																																//empty inputs
																																$("#inputUrl").val("");
																																$("#selectUrlflag").prop("selectedIndex",0);
																																$("#inputContent").val("");
																													...
																													function refreshUrlRules() {
																														...
																														var requestUrl = '/nmds-web/rest/urlrule';
																														...
																														var responseStream = Rx.Observable.fromPromise(
																															jQuery.getJSON(requestUrl)).flatMap(function(obj) {
																																...
																																var obj = x;
																																if (obj.urlFlag === -2) {
																																	obj.urlFlag = "블랙리스트(부분일치)";
																																} else if (obj.urlFlag === -1) {
																																	obj.urlFlag = "예외처리(부분일치)";
																																} else if (obj.urlFlag === 1) {
																																	obj.urlFlag = "예외처리(전체일치)";
																																} else {
																																	obj.urlFlag = "블랙리스트(전체일치)";
																																}
																															...
																															responseStream.subscribe(function(x) {
																																var item = "<tr> <td>" + x.id + "</td><td>" + x.url + "</td>"
																																+ "<td>" + x.urlFlag + "</td>" + "<td>" + x.content
																																+ "</td>" + "<td>" + x.input_dept + "</td>" + "<td>"
																																+ x.input_user + "</td>" + "<td>" + x.input_ip
																																+ "</td>" + "<td>" + x.timestamp + "</td><td></td></tr>"
																																$("#urlruletbody").append(item);
																																//console.log(x);
																																//console.log('Next: ' + x);
																															}, function(err) {
